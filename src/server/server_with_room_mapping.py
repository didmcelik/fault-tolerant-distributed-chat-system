...
# The "coordinator" TCP port is args.port (same as before).
# Room TCP ports are allocated from this range per server.
ROOM_PORT_BASE = 5100
ROOM_PORT_MAX = 5200
...
class ChatServer:
    ...
    def __init__(self, host: str, port: int):
        ...
        # Coordinator state (leader-maintained, gossiped to all)
        self.room_owner: Dict[str, str] = {}         # room -> server_id
        self.room_port: Dict[str, int] = {}          # room -> tcp port (on owner)
        self.server_load: Dict[str, int] = {self.server_id: 0}

        # Per-room TCP servers (local only)
        self._room_servers: Dict[str, asyncio.base_events.Server] = {}
        self._room_clients: Dict[str, Set[asyncio.StreamWriter]] = {}
        self._used_room_ports: Set[int] = set()
    ...
    def _alloc_room_port(self) -> int:
        for p in range(ROOM_PORT_BASE, ROOM_PORT_MAX + 1):
            if p not in self._used_room_ports:
                self._used_room_ports.add(p)
                return p
        raise RuntimeError("No free room ports available in configured range.")
    ...
    def _ensure_room_port_assigned(self, room: str, owner_sid: str) -> int:
        """
        Leader-only: assign a room TCP port for the room on its owner server.
        """
        if room in self.room_port:
            return self.room_port[room]

        # Allocate a port number (this is a logical assignment; owner will open it locally).
        # In this design, port numbers are per-server; it is okay if different servers reuse the same number.
        # So we don't need globally unique ports across servers.
        port = ROOM_PORT_BASE + (abs(hash(room)) % (ROOM_PORT_MAX - ROOM_PORT_BASE + 1))
        # Avoid collisions locally (only matters on the owner machine). Best-effort.
        # If collision, shift until free.
        shift = 0
        while True:
            ...
            if candidate != self.port:
                # Avoid coordinator port just in case range overlaps.
    ...
    async def _ensure_local_room_server(self, room: str) -> None:
        """
        If this server owns the room and the room server is not running, start it.
        """
        ...
        while True:
            ...
            if msg.get("type") == "DISCOVER":
                reply = {
                    ...
                    "server_port": self.port,  # coordinator port
                }
    ...
    # -----------------------------
    # LCR election (unchanged behavior)
    # -----------------------------
    ...
    async def server_control_listener(self) -> None:
        ...
        while True:
            ...
            if mtype == "SERVER_HELLO":
                ...
                if changed:
                    # If coordinator state changed, start local room servers we own
    ...
    # -----------------------------
    # Failure detection (room reassignment only on leader)
    # -----------------------------
    def _reassign_rooms_from_dead_server(self, dead_sid: str) -> None:
        ...
        for room in rooms_to_move:
            ...
            # room_port stays the same numeric value; it is interpreted on the new owner.
    ...
    # -----------------------------
    # Coordinator TCP endpoint (JOIN only)
    # -----------------------------
    ...
    async def _handle_coordinator_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        try:
            while True:
                ...
                # Leader assigns owner server + room port
                ...
                # If I am the owner, ensure the room server is running immediately
